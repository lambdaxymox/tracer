...
#[derive(Debug)]
pub struct SceneLightingObject {
    object: Box<dyn ModelSpaceLightingObject>,
    pub model_matrix: Matrix4x4<f32>,
    model_matrix_inv: Matrix4x4<f32>,
}

impl SceneLightingObject {
    pub fn new(object: Box<dyn ModelSpaceLightingObject>, model_matrix: Matrix4x4<f32>) -> Self {
        let model_matrix_inv = model_matrix.inverse().unwrap();
    ...
    fn lighting_query_world_space_to_model_space(&self, query: &LightingQuery) -> LightingQuery {
        let ray_incoming_model_space = (self.model_matrix_inv * query.ray_incoming.extend(0_f32)).contract();
        let point_model_space = (self.model_matrix_inv * query.point.extend(1_f32)).contract();

        LightingQuery::new(ray_incoming_model_space, point_model_space)
    }

    fn lighting_result_model_space_to_world_space(&self, result: &LightingResult) -> LightingResult {
        let ray_incoming_world_space = (self.model_matrix * result.ray_incoming.extend(0_f32)).contract(); 
        let point_world_space = (self.model_matrix * result.point.extend(1_f32)).contract();
        let emission = result.emission;

        LightingResult::new(ray_incoming_world_space, point_world_space, emission)
    }

    pub fn emit(&self, query: &LightingQuery) -> LightingResult {
        let query_model_space = self.lighting_query_world_space_to_model_space(query);
        let result_model_space = self.object.emit(&query_model_space);
        
        self.lighting_result_model_space_to_world_space(&result_model_space)
    }
